...
template <>
struct moe_gemm1_heuristic_dispatcher<ck_tile::bf16_t, ck_tile::pk_fp4_t, float, ck_tile::bf16_t, 2, true, false>
{
    static MoeKernel dispatch(int M, int N, int K, int block_m)
    {
        ...
        if (block_m == 16)
        {
            return moe_cktile2stages_gemm1_256x16x128x256_1x4_16x16x32_2perCU_1x32_HasBias_swiglu<ck_tile::bf16_t, ck_tile::pk_fp4_t, float, ck_tile::bf16_t>;
        }
        else if (block_m == 32)
        {
            return moe_cktile2stages_gemm1_256x32x256x256_1x4_16x16x32_2perCU_1x32_HasBias_swiglu<ck_tile::bf16_t, ck_tile::pk_fp4_t, float, ck_tile::bf16_t>;
        }
        else if (block_m == 64)
        {
            return moe_cktile2stages_gemm1_256x64x256x256_1x4_16x16x32_1perCU_1x32_HasBias_swiglu<ck_tile::bf16_t, ck_tile::pk_fp4_t, float, ck_tile::bf16_t>;
        }
        else
        {
            TORCH_CHECK(
...
template <>
struct moe_gemm2_heuristic_dispatcher<ck_tile::bf16_t, ck_tile::pk_fp4_t, float, ck_tile::bf16_t, 2, true, false>
{
    static MoeKernel dispatch(int M, int N, int K, int block_m)
    {
        // Apply shape heuristics to find a suitable kernel implementation.
